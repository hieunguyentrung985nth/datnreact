...
const Approve = () => {
  ...
  useEffect(() => {
    const fetch = async () => {
      const data = await ManagerApi.getApprovePosts(page);
  ...
  const openModalToEdit = (currentPost) => {
    ...
    setPreview("https://localhost:44354/contents/" + currentPost.banner);
    // ckeditor3.current.setData(currentPost.description);
    // ckeditor3.current.setData(currentPost.content);
  };
  ...
  const approve = async (postId) => {
    if (window.confirm("Duyệt bài viết?")) {
      await ManagerApi.approvePost(postId);
      const data = await ManagerApi.getApprovePosts(page);
      setPostList(data.data.data);
      setPager(data.data.pager);
    }
  ...
  const feed = useRef();
  const [postDeclined, setPostDeclined] = useState(null);

  const showDeclineModal = (postId) => {
    setPostDeclined(postId);
  };

  const decline =async () => {
    console.log(feed.current?.value);
    console.log(postDeclined);
    const user = AuthApi.getCurrentUser();
    var feeds = {
      content: feed.current?.value,
      postId: postDeclined,
      userId: user.id,
    };
    await ManagerApi.declinePost(postDeclined, feeds);
    feed.current.value = '';
    const ef = document.getElementById("close2");
    ef?.click();
    const data = await ManagerApi.getApprovePosts(page);
    setPostList(data.data.data);
    setPager(data.data.pager);
      ...
      <div id="layoutSidenav">
        ...
        <div id="layoutSidenav_content">
          <div className="container-fluid px-4">
            <h1 className="mt-4">Duyệt bài</h1>
